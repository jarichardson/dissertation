...
\subsection{Vent Intensity}
	\frame{\frametitle{Volcano organization in fields}
		...
		\begin{block}{}
		\begin{itemize}
			\item Size of volcanic field determined by a set contour (95\%)\\[-0.5em]
			\begin{equation*}
...
\subsection{Methods}
	...
	\column{0.75\textwidth}
		\begin{tikzpicture}
			...
			%\pause
			\node (img2) at (img1.center) [xshift=2cm] {\includegraphics[width=0.27\textwidth]{figures/defense/120-25W_08-40S_100m_cum_fordefense.eps}};
		\end{tikzpicture}\\[-1em]
		%\includegraphics[width=0.82\textwidth]{figures/defense/V19-V23_view-outlines_300dpi.png}\\[-0.5em]
		\mbox{\tiny \textit{CTX Image: G10\_022160\_1710\_XN\_09S120W (NASA/JPL-Caltech/MSSS)}}
...
\fi
	...
	%          V25, V17
		...
		But by probabilistically modeling age, possible ages can still be identified
		\end{block}
	\column{0.5\textwidth}
		\centering
	...
	\frame{\frametitle{VERRM}
		\begin{block}{\textbf{Step 2:} Calculate Recurrence Rate of Volcanism}
			...
			\begin{equation*}
			\text{RR}(t) = \frac{3}{\text{T}_{e-2} - \text{T}_{e+2}}
			\end{equation*}
		\end{block}\\[-1em]
...
\subsection{Results}
	...
	\column{0.45\textwidth}
		\begin{itemize}
			...
			\item Rate peaks at 150~Ma, producing 1 vent per 1-10~Myr (Median value: 1/4~events~Myr$^{-1}$)
			\item Rate decreased monotonically since, essentially 0~events~Myr$^{-1}$ today
		\end{itemize}
	\end{columns}
	\end{block}
	}
...
\subsection{Implications}
	...
	\begin{tikzpicture}
			...
			%\pause
			\node at (img1.center) [xshift=-1cm,yshift=0.3cm] {\Large ?};
			\draw[red,thick,dashed] (-2.3,1.4) -- (0.3,-0.8);
		\end{tikzpicture}
		\centering
	...
	\begin{enumerate}
		...
		\item Volcanism is now in haitus (Recurrence Rate is $\sim$0~events~Myr$^{-1}$)
	\end{enumerate}
	\end{columns}
	}
...
\section{Conclusions}
	...
	\begin{itemize}
			...
			\item Dating methods used on lavas can be combined to estimate rate of volcanism
